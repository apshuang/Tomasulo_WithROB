...
Tomasulo::Tomasulo() {
	...
	reorderBuffer = new ReorderBuffer(floatRegisters, integerRegisters);
	instructionDecoder = new InstructionDecoder(integerRegisters, floatRegisters, loadBuffer, storeBuffer, reservationStationADD, reservationStationMULT, reorderBuffer);
	commonDataBus = new CommonDataBus(integerRegisters, floatRegisters, loadBuffer, storeBuffer, reservationStationADD, reservationStationMULT, instructionDecoder, reorderBuffer);
	outputTable.clear();
	cycle = 0;
}
...
bool Tomasulo::CheckStop() {
	...
	if (!reorderBuffer->isAllFree())return false;
	return true;
}
