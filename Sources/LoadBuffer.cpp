...
void LoadBufferLine::SetExecute() {
	...
	CommonDataBus::TransmissExecuteSignal(GetName());
}
...
void LoadBufferLine::Tick() {
	...
	if (remainingTime == 0) {
		...
		CommonDataBus::TransmissValue(GetName(), valueString);  // 在exec的最后传给CDB，CDB就会在下一个周期的开始，将这个result传给其它组件
	}
...
void LoadBuffer::BackTrackClear(string unitName) {
	for (int i = 0; i < LOADNUM; i++) {
		if (unitName == loadbuffers[i]->GetName()) {
			loadbuffers[i]->Reset();
		}
	}
}
...
void StoreBufferLine::Tick() {
	...
	remainingTime--;
	if (remainingTime == 0) {
		CommonDataBus::TransmissValue(GetName(), valueString);  // 实际上不占用CDB，只是将这个消息告诉ROB（如果采用ROB）
	}
	if (remainingTime == -1) {
		Reset();
	}
...
void StoreBuffer::BackTrackClear(string unitName) {
	for (int i = 0; i < STORENUM; i++) {
		if (unitName == storebuffers[i]->GetName()) {
			storebuffers[i]->Reset();
		}
	}
}
