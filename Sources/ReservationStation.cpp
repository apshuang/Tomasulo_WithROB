...
void ReservationStationLine::Tick() {
	...
	if (remainingTime == 0) {
		...
		CommonDataBus::TransmissValue(GetName(), valueString);
	}
	if (remainingTime == -1) {
...
void ReservationStationADD::BackTrackClear(string unitName) {
	for (int i = 0; i < ADDNUM; i++) {
		if (unitName == addModule[i]->GetName()) {
			addModule[i]->Reset();
		}
	}
}
...
void ReservationStationMULT::BackTrackClear(string unitName) {
	for (int i = 0; i < MULTNUM; i++) {
		if (unitName == multModule[i]->GetName()) {
			multModule[i]->Reset();
		}
	}
}
